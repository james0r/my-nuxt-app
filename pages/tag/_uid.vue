<template>
  <div>
    <PageTitle
      :title="`Posts Tagged: ${$route.params.uid}`"
      :subtitle="`The posts below match the tag ${$route.params.uid} and are ordered by the original publication date.`"
    />
...
export default {
  ...
  async asyncData({ $prismic, params, error }) {
    const { id } = await $prismic.api.getByUID('tag', params.uid)
    const documents = await $prismic.api.query(
      $prismic.predicates.at('my.post.tags.tag', id),
      {
